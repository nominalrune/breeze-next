...
function TaskItem({ task: _task, user, update }: { task: TaskDTO, user: any; update: (task: TaskDTO) => Promise<TaskDTO>; }) {
			...
			<div className='md:grid  md:grid-rows-[7rem] md:grid-cols-6 gap-2 md:h-28 overflow-hidden'>
				...
				<div className='md:col-span-2  md:flex flex-row'>
					<div className='m-1 grow border-2 rounded-md overflow-scroll text-base font-mono'>
						<TaskTree tasks={task.subtasks ?? []} update={handleSubtaskUpdate} isEditing={isEditing} />
					</div>
	...
	return show ? (<>
		<div className='absolute right-20 p-1 w-48 text-slate-500 bg-white rounded-md shadow-md text-center'>
			<div className='border-b-[1px] border-slate-200'>
				<SwitchStateButtons task={task} updateState={(newState: number) => update({ ...task, state: newState }, true).then(() => { toast.success('Task Updated!'); }, () => { toast.error('Oops, Some error occurred'); })} />
			</div>
